...
def get_cars(request):
    try:
        ...
        # Retrieve all car models along with their related CarMake using
        # select_related
        ...
        logger.error(
            "Database table not found. Have you run migrations?",
            exc_info=True)
        return json_response(
            {"error": "Failed to retrieve car models. Have you run migrations?"}, status=500)
    except Exception as e:
        logger.error(f"Exception in get_cars: {e}", exc_info=True)
        return json_response(
            {"error": "Failed to retrieve car models"}, status=500)
        ...
        if reviews is not None:
            for review_detail in reviews:
                ...
                review_detail['sentiment'] = sentiment.get(
                    'sentiment', 'neutral') if sentiment else 'neutral'
            logger.debug(
                f"Retrieved {len(reviews)} reviews for dealer ID: {dealer_id}")
...
def add_review(request):
    ...
    try:
        ...
        # Check if the returned JSON contains an 'id' (i.e. the review was
        # saved)
        if response and "id" in response:
            ...
            logger.error(
                "Failed to post review via backend API. Response: " +
                str(response))
            return json_response(
