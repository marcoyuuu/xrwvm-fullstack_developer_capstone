...
def get_request(url, **kwargs):
    try:
        ...
        logger.debug(
            f"GET request successful. Status Code: {response.status_code}")
...
def post_review(data_dict):
    ...
    required_fields = [
        ...
        "car_year"]
    missing_fields = [
        field for field in required_fields if field not in data_dict]
    if missing_fields:
    ...
    try:
        ...
        response.raise_for_status()
        logger.debug(
            f"POST request successful. Status Code: {response.status_code}")
        return response.json()
    except RequestException as e:
        ...
        logger.error(
            f"JSON decode error for POST request to {request_url}: {e}")
